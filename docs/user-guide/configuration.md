...
Set the remote prefix to push to when using `gh-deploy` to deploy to GitHub Pages.
This option can be overridden by a command line option in `gh-deploy`.

WARNING:
It is not possible to deploy a site configured with a `remote_prefix` alongside an
un-prefixed site in the same `remote_branch`. The site deployed at the root of the
branch will delete the prefixed site as they are unaware of each other.
